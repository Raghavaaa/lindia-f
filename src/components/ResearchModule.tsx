...
export default function ResearchModule({ clientId, onResearchComplete }: Props) {
  ...
  async function runResearch() {
    ...
    try {
      ...
      setProgressMessage("AI is processing your query...");
      ...
      const response = await apiFetchWithRetry(config.endpoints.research, {
        method: 'POST',
        body: JSON.stringify({
          query: query.trim(),
          ...
          <Card>
            ...
            {showResults && (
              <CardContent className="space-y-4">
                ...
                {currentResult && (
                  <div className="mt-6 p-6 bg-gradient-to-br from-muted/50 to-muted/30 rounded-xl border border-border shadow-sm">
                    <div className="flex items-start justify-between mb-4">
                      <h3 className="font-semibold text-lg text-primary">
                        Research Result
                      </h3>
                      <span className="text-xs text-muted-foreground">
                        {formatTime(currentResult.ts)}
                      </span>
                    </div>
                    <h4 className="font-medium text-foreground mb-3 text-base">
                      {currentResult.title}
                    </h4>
                    <div className="bg-background/80 rounded-lg p-4 mb-4 max-h-[600px] overflow-y-auto">
                    ...
                    {currentResult.adminPrompt && (
                      <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg border border-blue-200 dark:border-blue-800">
                        <p className="text-xs font-medium text-blue-800 dark:text-blue-300 mb-1">Admin Prompt:</p>
                        <p className="text-xs text-blue-700 dark:text-blue-400">{currentResult.adminPrompt}</p>
                      </div>
                    )}
                  </div>
                )}
